...
function Media() {
  ...
  const makeTitleStreamable=(str:string)=>{
    const res=str.toLowerCase().split(' ').join('-');
    return res;
  }
  ...
  return (
    data.Media &&(
            ...
            <div className="absolute inset-y-28 md:inset-y-auto md:bottom-10 inset-x-4 md:inset-x-12 space-y-6 z-50">
              ...
              <p className="text-xs md:text-sm">
                  <>
                   ...
                   <span className="font-semibold text-base"> {titleCase(data.Media?.season!)}</span> 
                  </>
                {" "}
                {" "}
      ...
      <ButtonDisplay id={id} episodes={data.Media.episodes==null?2000:data.Media.episodes}/>
      </div>
    )
  );
}
